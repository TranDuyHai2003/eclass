...
export function HomeSidebar({ user: propUser }: HomeSidebarProps) {
  ...
  const generalLinks = [
    ...
    { icon: DiscordIcon, label: "Discord", href: "https://discord.gg/eclass", target: "_blank" },
  ];
  ...
  const socialLinks = [
    { icon: Facebook, label: "Facebook", href: "https://facebook.com", color: "text-blue-600 hover:text-blue-700" },
    { icon: MessageCircle, label: "Messenger", href: "https://messenger.com", color: "text-pink-500 hover:text-pink-600" },
    { icon: Youtube, label: "Youtube", href: "https://youtube.com", color: "text-red-600 hover:text-red-700" }
  ];
      ...
      return (
        <Link
          ...
          className={cn(
            ...
            isActive 
              ? "bg-[#FFF1F2] text-red-600 shadow-sm" 
              : "hover:bg-[#FFF1F2]/50 text-slate-600 hover:text-red-600"
          )}
        >
          <div className="flex items-center gap-3">
            <div className={cn(
              ...
              isActive 
                ...
                : "bg-white border-slate-100 text-slate-400 group-hover:text-red-600 group-hover:border-red-100"
            )}>
          ...
          <ChevronRight className={cn(
            ...
            isActive ? "text-red-600 opacity-100" : "text-slate-300 group-hover:text-red-600 group-hover:translate-x-0.5"
          )} />
        </Link>
      );
    });
  ...
  return (
    <div className="w-full bg-transparent overflow-hidden p-2 space-y-6">
        ...
        <nav className="flex flex-col gap-1 mt-1">
          {socialLinks.map((item) => (
            <a
              ...
              className="flex items-center gap-3 p-2.5 rounded-xl hover:bg-[#FFF1F2]/40 transition-all text-slate-600 hover:text-red-600 group"
            >
              <div className={cn("p-1.5 rounded-lg bg-white border border-slate-100 group-hover:border-red-100 transition-colors", item.color)}>
                <item.icon className="w-4 h-4" />
              </div>
      ...
      {mounted && typeof window !== "undefined" && showDiscordModal && createPortal(
        <div className="fixed inset-0 z-[999999] flex items-center justify-center p-4 animate-in fade-in duration-200">
          ...
          {/* Dialog Container */}
          <div className="relative bg-white/95 backdrop-blur-xl rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl border border-red-100/50 transform transition-all duration-300 scale-100 animate-in zoom-in-95">
            {/* Header Icon */}
            ...
            <div className="space-y-3">
              ...
              <p className="text-slate-500 text-xs font-bold leading-relaxed uppercase tracking-tight">
                Bạn chuẩn bị tham gia vào cộng đồng học tập trực tuyến eClass trên Discord để cùng nhau trao đổi bài giảng, thảo luận bài tập về nhà và nhận hỗ trợ nhanh chóng từ trợ giảng!
              </p>
            </div>
            ...
            <div className="mt-8 flex flex-col sm:flex-row gap-3">
              <a 
                ...
                rel="noopener noreferrer"
                onClick={() => setShowDiscordModal(false)}
                className="flex-1 py-4 bg-[#A01D24] text-white rounded-2xl text-[10px] font-black uppercase tracking-widest text-center shadow-lg hover:bg-slate-900 hover:shadow-xl active:scale-95 transition-all animate-none"
              >
