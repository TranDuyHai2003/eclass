...
export function PersonalAnalytics() {
  ...
  // If not eligible or no data, hide the component entirely
...
return (
  <div className="bg-white rounded-[3rem] p-4 sm:p-6 space-y-10 relative overflow-hidden">
    <div className="absolute top-0 right-0 w-96 h-96 bg-blue-50/50 blur-[100px] rounded-full pointer-events-none" />
    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative z-10">
           ...
           >
                  ...
                  return (
                    <div className="bg-slate-900 p-4 rounded-2xl shadow-xl border border-white/10 text-white space-y-1">
                      ...
                      <div className="flex justify-between gap-8 text-[10px] opacity-70">
                         ...
                         <span className={cn("font-bold", (100-d.error_rate) > 70 ? "text-emerald-400" : "text-blue-400")}>
             ...
             <Bar dataKey="val" radius={[0, 8, 8, 0]} barSize={24}>
               {chartData.map((entry, index) => (
                 <Cell key={`cell-${index}`} fill={entry.val < 50 ? '#3b82f6' : '#10b981'} fillOpacity={0.8} />
               ))}
             </Bar>
           </BarChart>
         </ResponsiveContainer>
      ...
      <div className="lg:col-span-5 space-y-6">
            ...
            {data.weaknesses.length > 0 ? (
              <div className="space-y-3">
                 {data.weaknesses.map((w, i) => (
                    <div key={w.category} className="group p-5 bg-white border border-slate-100 rounded-3xl shadow-sm hover:border-blue-200 hover:shadow-lg transition-all">
                       <div className="flex items-start gap-4">
                          <div className="w-10 h-10 rounded-2xl bg-blue-50 flex items-center justify-center text-blue-600 shrink-0 group-hover:bg-blue-600 group-hover:text-white transition-colors shadow-sm">
                          ...
                          <div className="flex-1 min-w-0">
                             ...
                             <div className="flex items-center justify-between">
                                ...
                                <Link 
                                  ...
                                  className="inline-flex items-center gap-1 text-[10px] font-black text-slate-400 hover:text-blue-600 transition-colors"
                                >
                                   ÔN TẬP NGAY
         ...
         <div className="p-6 bg-slate-900 rounded-[2.5rem] text-white relative overflow-hidden group shadow-xl shadow-blue-900/10">
            ...
            <p className="text-sm font-bold leading-relaxed mb-4">
              Dựa trên dữ liệu, bạn nên dành <span className="text-blue-400">15 phút/ngày</span> để làm lại các câu sai thuộc dạng <span className="text-white underline underline-offset-4">{data.weaknesses[0]?.category || "mới"}</span>.
            </p>
            <Button asChild variant="outline" className="w-full rounded-2xl border-white/10 bg-white/5 hover:bg-white/10 text-white font-black text-xs uppercase tracking-tight h-11 transition-all">
