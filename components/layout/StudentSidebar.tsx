...
const SidebarItem = ({ href, icon: Icon, label, isActive }: SidebarItemProps) => (
  ...
  >
    ...
    <ChevronRight className={cn(
      ...
      isActive ? "text-blue-600 translate-x-0" : "text-slate-300 group-hover:text-blue-600 group-hover:translate-x-0.5"
    )} />
  </Link>
);
  ...
  return (
    <aside className="w-full lg:w-[280px] flex flex-col gap-5 shrink-0">
      ...
      <div className="bg-white rounded-[2rem] p-6 shadow-sm border border-slate-100 flex flex-col items-center text-center">
        <div className="w-20 h-20 rounded-full bg-blue-100 ring-4 ring-white shadow-xl flex items-center justify-center mb-4 relative overflow-hidden group">
          {user?.image ? (
            ...
            <div className="w-full h-full bg-gradient-to-br from-blue-600 to-orange-500 flex items-center justify-center text-white text-2xl font-black">
              {user?.name?.[0] || "U"}
            </div>
          )}
        </div>
        ...
        <div className="mt-4 px-3 py-1 bg-blue-50 text-blue-600 rounded-full text-[9px] font-black uppercase tracking-[0.15em] border border-blue-100">
