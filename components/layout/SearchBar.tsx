...
export function SearchBar() {
  ...
  return (
    <div ref={containerRef} className="flex-1 max-w-xl relative group z-[9999]">
      <div className="absolute inset-0 bg-blue-600/5 blur-xl rounded-full opacity-0 group-focus-within:opacity-100 transition-opacity pointer-events-none" />
      ...
      <form onSubmit={handleSearch} className="relative z-10 flex items-center w-full">
        <input
          ...
          className="w-full h-11 sm:h-12 pl-6 pr-24 rounded-2xl bg-slate-50 border border-slate-200 focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 focus:outline-none transition-all text-sm font-bold text-slate-700 shadow-inner"
        />
        ...
        <div className="absolute right-3 top-1/2 -translate-y-1/2 flex items-center gap-2 z-20">
          ...
          <button
            type="submit"
            className="w-8 h-8 sm:w-9 sm:h-9 bg-slate-900 rounded-xl flex items-center justify-center text-white hover:bg-blue-600 transition-all shadow-lg active:scale-95"
          >
            ...
            {isLoading ? (
              ...
              <>
                <div className="px-2 py-1 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-slate-50 flex items-center justify-between">
                  ...
                  <span className="text-blue-600">eClass Course</span>
                </div>
                ...
                <div className="flex flex-col gap-1.5">
                  {results.slice(0, 5).map((course) => (
                    <Link
                      ...
                      className="flex items-center gap-4 p-2.5 rounded-2xl hover:bg-blue-50/50 transition-all group"
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-0.5">
                          {course.category?.name && (
                            <span className="px-2 py-0.5 bg-blue-50 text-blue-600 rounded-md text-[8px] font-black tracking-widest uppercase">
                              {course.category.name}
                            </span>
                          )}
                          {course.user?.name && (
                        ...
                        <h4 className="font-black text-[13px] text-slate-800 line-clamp-1 uppercase tracking-tight group-hover:text-blue-600 transition-colors">
                          {course.title}
                        </h4>
                      </div>
                      
                      <div className="shrink-0 text-right pr-2">
                        <span className="font-black text-xs text-slate-950 group-hover:text-blue-600 transition-colors">
